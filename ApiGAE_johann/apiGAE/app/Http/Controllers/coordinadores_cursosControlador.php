...

{
    ...
    public function store(Request $request){
        ...
        $coordinadores_cursos = coordinadores_cursosModelo::create([
            'id_coordinadores_cursos' => $request->id_coordinadores_cursos, // Asegúrate de que esto coincida con lo que envías. Antes tenías $request->codigoCargos
        ...
        if(!$coordinadores_cursos){
            $data = [
                'message' => 'Error al crear el coordinador_curso',
                'status' => 500
            ];
        ...
        $data = [
            'message' => 'coordinador_curso creado Correctamente',
    ...
    public function show($id_coordinadores_cursos){
        ...
        if(!$coordinadores_cursos){ 
            $data = [
                'message' => 'cordinador_curso no encontrado',
                'status' => 404
            ];
            return response()->json($data, 404);
        }
    ...
    public function destroy($id_coordinadores_cursos){
        ...
        if(!$coordinadores_cursos){
            $data = [
                'message' => 'coordinador_curso no encontrado', 
                'status' => 404
            ];
            return response()->json($data, 404);
        }
        ...
        $data = [
            'message' => 'coordinador_curso Eliminado',
            'status' => 200
        ];
        return response()->json($data, 200);
    }
    ...
    public function update(Request $request, $id_coordinadores_cursos){
       ...
       if(!$coordinadores_cursos){
        ...
        $data = [
            'message' => 'coordinadores_curso Actualizado',
            'coordinador_curso' => $coordinadores_cursos,
            'status' => 200
        ];
