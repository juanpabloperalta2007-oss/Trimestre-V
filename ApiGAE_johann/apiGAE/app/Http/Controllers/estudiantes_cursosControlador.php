...
class estudiantes_cursosControlador extends Controller
{
    ...
    public function store(Request $request){
        ...
        // CORRECCIÓN: Uso de cargosModelo y arreglo de la flecha ->
        $estudiantes_cursos = estudiantes_cursosModelo::create([
            'id_estudiante_curso' => $request->id_estudiante_curso, // Asegúrate de que esto coincida con lo que envías. Antes tenías $request->codigoCargos
        ...
        if(!$estudiantes_cursos){
            $data = [
                'message' => 'Error al crear el Estudiante_curso',
                'status' => 500
            ];
        ...
        $data = [
            'message' => 'estudiante_curso creado Correctamente',
    ...
    public function show($id_estudiante_curso){
        ...
        if(!$estudiantes_cursos){ 
            $data = [
                'message' => 'estudiante_curso no encontrado',
                'status' => 404
            ];
            return response()->json($data, 404);
        }
    ...
    public function destroy($id_estudiante_curso){
        ...
        $data = [
            'message' => 'estudiante_curso Eliminado',
            'status' => 200
        ];
        return response()->json($data, 200);
    }
    ...
    public function update(Request $request, $id_estudiante_curso){
       ...
       if(!$estudiantes_cursos){
        ...
        $data = [
            'message' => 'estudiante_curso Actualizado',
            'estudiante_curso' => $estudiantes_cursos,
            'status' => 200
        ];
