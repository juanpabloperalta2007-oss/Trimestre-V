...
class cargosControlador extends Controller
{
    ...
    public function store(Request $request){
        ...
        $cargos = cargosModelo::create([
            'id_cargo' => $request->id_cargo, // Asegúrate de que esto coincida con lo que envías. Antes tenías $request->codigoCargos
    ...
    public function destroy($codigocargos){
        ...
        if(!$cargos){
            $data = [
                'message' => 'Cargo no encontrado', // Corregido: Decía "cargo encontrado"
                'status' => 404
            ];
            return response()->json($data, 404);
        }
